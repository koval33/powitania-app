...
const { logInquiry } = require('./inquiry-logger');

// ─── Configuration ───────────────────────────────────────────────────────────
const CRM_WEBHOOK_URL = process.env.CRM_WEBHOOK_URL || 'https://your-crm.railway.app';
const CRM_API_KEY = process.env.CRM_API_KEY || '';
// 25s, nie 10s: CRM potrafi odpowiadac wolno, a nasz timeout konczyl sie cichym
// porzuceniem leada (kazda porazka nizej rozwiazuje sie, nie rzuca wyjatkiem).
const CRM_WEBHOOK_TIMEOUT = parseInt(process.env.CRM_WEBHOOK_TIMEOUT || '25000', 10);
 ...
 */
async function sendOrderToCRMRequest(orderData) {
    ...
    // Atrybucja zrodla zamowienia (z cookie pow_attrib -> formularz -> orderData).
    // Doklada tylko niepuste - CRM przyjmuje te pola (wdrozone po stronie CRM).
    ['gclid', 'utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'fbclid', 'gaClientId'].forEach(function (k) {
        if (orderData[k]) payload[k] = orderData[k];
    });
...
/**
 * Wysylka do CRM + ZAPIS WYNIKU do logu zapytan.
 *
 * Po co: kazda sciezka bledu w sendOrderToCRMRequest konczy sie resolve(), nigdy
 * reject(), wiec .catch() w miejscu wywolania nigdy sie nie odpala, a jedynym
 * sladem awarii byla linia w logach Railway. Efekt: nie dalo sie odpowiedziec na
 * pytanie "czemu to zamowienie nie weszlo do CRM" bez archeologii w logach.
 *
 * Teraz kazda proba zostawia wpis type:'crm' z crm_status (sent/failed/skipped),
 * crm_error, deal_id i czasem odpowiedzi - widoczny w /admin/lektorzy/inquiries/.
 * Logowanie nie moze zepsuc glownego flow, stad try/catch dookola.
 */
async function sendOrderToCRM(orderData) {
    const started = Date.now();
    let result;
    try {
        result = await sendOrderToCRMRequest(orderData);
    } catch (err) {
        result = { ok: false, error: String((err && err.message) || err) };
    }

    try {
        const status = result && result.ok ? 'sent' : (result && result.skipped ? 'skipped' : 'failed');
        logInquiry({
            type: 'crm',
            data: {
                name: orderData.name || '',
            ...
            crm_status: status,
            crm_error: status === 'sent' ? null : String((result && (result.error || result.message)) || 'unknown')
        });
    } catch (e) {
        console.error('[CRM] Nie udalo sie zapisac wyniku do logu:', e.message);
    }

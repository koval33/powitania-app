...
function verifyTurnstile(req, res, next) {
  ...
  if (!token) {
    // Fail open - brak tokena może oznaczać Safari z iCloud Private Relay lub
    // inny browser blokujący widget Turnstile (np. blokery reklam).
    // Rate limiter na poziomie routy chroni przed nadużyciem.
    console.log('[turnstile] Missing token (fail open) from', req.ip);
    return next();
  }
